import IBoardPresenterProps from "./comment_edit.presenter";
import { useMutation } from "@apollo/client";
import { UPDATE_BOARD_COMMENT } from "./comment_edit.querise";
import { ChangeEvent, useState } from "react";
import { useRouter } from "next/router";
// import { IBoardContainerProps, INewInputs } from "./comment_write.types";

import {} from "./comment_edit.types";
import {
	IMutation,
...
export default function CommentEditContainer() {
	...
	async function onClickUpdate(event) {
		...
		try {
			...
			// router.push(`/boards/board_input/${result.data.updateBoardComment._id}`);
		} catch (error) {
			alert(error.message);
		}
	}
